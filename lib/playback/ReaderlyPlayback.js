...
(function (root, playbackFactory) {  // root is usually `window`
	...
	var ReaderlyPlayback = function ( timer, displayCore ) {
		...
		var progressNode, percentDone, scrubber, progSlider;
...
// </div>';
		...
		rPly._showProgress = function ( evnt, timer, fraction, indx ) {
			...
			// progSlider.set( indx );  // version 8 nouislider
			return rPly;
		};
		...
		*/
			...
			// progNode.noUiSlider.on('update', function updatePos( values, handle ) {
			// 	// Change the index to whatever this says
			// 	// data.setFunc( values[handle] );
			// });
		...
		rPly._addEvents = function () {
			...
			// DOM events
			// progressNode.noUiSlider.on('update', function updatePos( values, handle ) {
			// 	// Change the index to whatever this says
			// 	// data.setFunc( values[handle] );
			// });
