...
(function (root, Queue) {  // root is usually `window`
    if (typeof define === 'function' && define.amd) {  // amd if possible
        // AMD. Register as an anonymous module.
        define( [], function () { return (root.Queue = Queue()); });
    ...
    */
        ...
        */
        ...
        // ========= TRAVELING THE QUEUE (for external use) ========= \\
        qu.getWord = function () {
            if (qu.fragments.length && qu.index < qu.fragments.length)
                return qu.fragments[qu.index];
            else
                return null;
        };
        ...
        qu.next = function () {
            ...
            qu.index = Math.min( qu.index + 1, qu.fragments.length );  // prep for next request
            return frag;
        };

        qu.prev = function () {
            var frag = qu.fragments[ qu.index ];
            qu.index = Math.max( qu.index - 1, 0 );  // prep for next request
            return frag;
        };
        ...
        qu.getProgress = function () {
            return qu.index / qu.fragments.length;
        };
