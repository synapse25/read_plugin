	...
	*/
		...
		rTim._setUp = function ( settings ) {
			...
			var wpm 	= settings.wpm 			  || _rSetts.wpm,
				start 	= settings.slowStartDelay || _rSetts.slowStartDelay,
			...
			rTim.setwpm( wpm ).setslowstartdelay( start )
				.setsentencedelay( sentDel ).setotherpuncdelay( puncDel )
				.setshortworddelay( shWrDel ).setlongworddelay( lgWrDel )
				.setnumericdelay( numDel );
		...
		rTim.settingsAvailable = ['wpm', 'sentenceDelay', 'otherPuncDelay', 'shortWordDelay',
						'longWordDelay', 'numericDelay', 'slowStartDelay'];  // Not needed, but might be nice to have
